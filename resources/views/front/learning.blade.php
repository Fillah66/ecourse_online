    ...
    <section id="video-content" class="max-w-[1100px] w-full mx-auto mt-[130px]">
        <div class="video-player relative flex flex-nowrap gap-5">
            <div class="plyr__video-embed w-full overflow-hidden relative rounded-[20px]" id="player">
                <iframe src="/images/materi/{{ $video->path_video }}#toolbar=0&navpanes=0&scrollbar=0&embedded=true"
            ...
            <div
                ...
                <p class="font-bold text-lg text-black">{{ $videos->count() }} Lessons</p>
                <div class="flex flex-col gap-3">
                    ...
                    @foreach ($videos as $item)
                        @php
                            $currentVideoId = request()->route('courseVideoId');
                            $isActive = $currentVideoId == $item->id;
                            $previous = $videos->where('id', '<', $item->id)->last();
                            $canAccess = true;
                            if ($previous) {
                                $canAccess = \App\Models\MateriProgress::where('user_id', auth()->id())
                                    ->where('course_video_id', $previous->id)
                                    ->where('is_completed', true)
                                    ->exists();
                            }

                            $progress = \App\Models\MateriProgress::where('user_id', auth()->id())
                                ->where('course_video_id', $item->id)
                                ->first();
                        @endphp
                        <div
    ...
    <section id="Video-Resources" class="flex flex-col mt-5">
        <div class="max-w-[1100px] w-full mx-auto flex flex-col gap-3">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <form action="{{ route('materi.complete', $video->id) }}" method="POST">
                        @csrf
                        <button
                            class="px-5 py-2 bg-[#3525B3] p-2 rounded-full text-white rounded-lg hover:bg-[#E9EFF3] transition-all duration-300">
                            Selanjutnya
                        </button>
                    </form>
                </div>
